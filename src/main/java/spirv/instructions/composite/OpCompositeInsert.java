...
public class OpCompositeInsert implements CompositeInstruction, ResultType, Result {
	...
	public void load(int[] data, int offset, int length, Instruction[] instructions) {
		...
		this.composite = instructions[data[offset + 3]];
		this.indexes = new int[length - 4];
		for (int i = 0; i < this.indexes.length; i++) {
			this.indexes[i] = data[offset + 4 + i];
		}
	}
	...
	@Override
	public int opcode() {
		return OpCodes.OP_COMPOSITE_INSERT;
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof OpCompositeInsert)) {
			return false;
		}
